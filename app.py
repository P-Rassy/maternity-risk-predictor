...
def to_str_array(x):
    """Convert array to string for OneHotEncoder."""
    x = pd.DataFrame(x).astype(object)
    x = x.where(~x.isna(), "__MISSING__")
    return x.astype(str).values

# ── Patch it into __main__ so joblib can find it ──────────────────────────────
import sys
import types
main_module = sys.modules["__main__"]
if not hasattr(main_module, "to_str_array"):
    setattr(main_module, "to_str_array", to_str_array)
...
st.markdown(
    "Enter the patient's clinical measurements from the **first visit** "
    "and **third trimester**, then click **PREDICT** to get the risk assessment."
)
st.warning(
    "⚠️ This tool is intended for **decision support only**. "
    "All clinical decisions should remain with the treating clinician."
)
...
if st.button("🔍 PREDICT RISK", use_container_width=True):
    ...
    eng = {
        ...
        "pregnancy_loss_rate":                  max(0, min(1, (prev_prg - prev_dlv) / prev_prg))
                                                if prev_prg > 0 else np.nan,
    ...
    if prob >= highrisk_thr:
        st.error(
            f"🔴 **HIGH RISK**\n\n"
            f"Complication Probability: **{prob:.1%}**\n\n"
            f"Action: This patient requires **URGENT** clinical review "
            f"and a care plan before delivery."
        )
        ...
        st.warning(
            ...
            f"Follow up closely during and after delivery."
        )
    else:
        st.success(
            f"🟢 **LOW RISK**\n\n"
    ...
    with col6:
        ...
        st.metric("Depression Risk PHQ-2","Positive ⚠️" if phq2_3rd >= 3 else "Negative ✅")
...
with col3:
    eq5d_ht_1st  = st.slider("EQ-5D Health Today — 1st Visit",        0, 100, 80)
    eq5d_idx_1st = st.slider("EQ-5D Index — 1st Visit",      -59, 100, 80)
with col4:
    eq5d_ht_3rd  = st.slider("EQ-5D Health Today — 3rd Trimester",    0, 100, 75)
    eq5d_idx_3rd = st.slider("EQ-5D Index — 3rd Trimester", -59, 100, 75)
