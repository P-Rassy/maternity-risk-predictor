...
# ── CRITICAL: define custom function used by the saved model ──────────────────
def to_str_array(x):
...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(page_title="Maternity Risk Predictor", page_icon="🤱", layout="centered")
...
st.markdown("Enter the patient measurements then click **PREDICT**.")
...
if st.button("🔍 PREDICT RISK", use_container_width=True):
    ...
    eng = {
        "height_cm__first_visit": height, "weight_kg__first_visit": w1,
        "weight_kg__third_trimester": w3, "prev_c_sections__first_visit": prev_cs,
        "prev_deliveries__first_visit": prev_dlv, "prev_pregnancies__first_visit": prev_prg,
        "phq2_total__first_visit": phq2_1st, "phq2_total__third_trimester": phq2_3rd,
        "eq5d_index__first_visit": eq5d_idx_1st, "eq5d_index__third_trimester": eq5d_idx_3rd,
        "eq5d_3l_healthtoday__first_visit": eq5d_ht_1st,
        "eq5d_3l_healthtoday__third_trimester": eq5d_ht_3rd,
        "wexner_total__first_visit": wexner_1st, "wexner_total__third_trimester": wexner_3rd,
        "iciq_total__first_visit": iciq_1st, "iciq_total__third_trimester": iciq_3rd,
        "bmi_first_visit": bmi1, "bmi_third_trimester": bmi3,
        "weight_gain": wg,
        ...
        "obese_at_booking": float(bmi1 >= 30), "underweight_at_booking": float(bmi1 < 18.5),
        "excessive_weight_gain": float(wg > 16),
        "phq2_change": phq2_3rd - phq2_1st, "eq5d_index_change": eq5d_idx_3rd - eq5d_idx_1st,
        "wexner_change": wexner_3rd - wexner_1st, "iciq_change": iciq_3rd - iciq_1st,
        "phq2_positive_1st": float(phq2_1st >= 3), "phq2_positive_3rd": float(phq2_3rd >= 3),
        "poor_health_1st": float(eq5d_ht_1st < 50), "poor_health_3rd": float(eq5d_ht_3rd < 50),
        "any_bowel_symptoms_1st": float(wexner_1st > 0), "any_bowel_symptoms_3rd": float(wexner_3rd > 0),
        "age_over_35": np.nan,
    }
